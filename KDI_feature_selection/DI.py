...
from KDI_feature_selection import kernel
def center(X):

    mean_col = tf.compat.v1.reduce_mean(X, axis=0, keep_dims=True)
    mean_row = tf.compat.v1.reduce_mean(X, axis=1, keep_dims=True)
    mean_all = tf.compat.v1.reduce_mean(X)
...
#                raw_kernel = 1/5*(kernel_X1(X*self.w)+kernel_X2(X*self.w)+kernel_X3(X*self.w)+kernel_X4(X*self.w)+kernel_X5(X*self.w))
                ...
                G_X_w_inv = tf.compat.v1.matrix_inverse(G_X_w)

            self.loss = tf.compat.v1.trace(tf.matmul(G_X_w_inv, G_B_w))-mu*tf.reduce_sum(tf.multiply(self.w, 1-self.w))
