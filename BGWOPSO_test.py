...
def fitness(x, X, y):
    ...
    for i in range(x.shape[0]):
        if np.sum(x[i, :]) > 0:
            score = cross_val_score(KNeighborsClassifier(n_neighbors=5), X[:, x[i, :].astype(bool)], y, cv=k_fold)
...
for i in range(len(filepath)):
    ...
    g = 70
    p = 8
    loss = functools.partial(fitness, X=X_minmax, y=Y)
    optimizer = BHPSOGWO(fit_func=loss, num_dim=X_minmax.shape[1], num_particle=p, max_iter=g)
    optimizer.opt()
    dataset = X_minmax[:, optimizer.gBest_X.astype(bool)]
