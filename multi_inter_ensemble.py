...
# -*- coding: UTF-8 -*-
#@author: xiaoshi
#@file: multi_inter_ensemble
#@Time: 2021/12/22 10:52
import time
import os
import warnings
...
for i in range(len(filepath)):
    ...
    '''多重交集'''
    inter1 = list(set(dt_index[0:n]) & set(gbdt_index[0:n]))
    inter2 = list(set(dt_index[0:n]) & set(rfc_index[0:n]))
    inter3 = list(set(rfc_index[0:n]) & set(gbdt_index[0:n]))
    multi_inter1 = list(set(inter1).union(set(inter2)))
    multi_inter = list(set(multi_inter1).union(set(inter3)))
    multi_inter.sort()

    feature_len = len(multi_inter)
    ...
    '''把选择的基因保存到文件中'''
    gene_index = [[filename,multi_inter]]
